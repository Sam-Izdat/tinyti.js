...
class CompilingVisitor extends ASTVisitor<Value> {
    ...
    protected override visitBinaryExpression(node: ts.BinaryExpression): VisitorResult<Value> {
        ...
        if (opToken.kind === ts.SyntaxKind.EqualsToken) {
            ...
            if (this.isInVertexFor() && leftPointerType.getIsGlobal()) {
                this.errorNode(
                    node,
                    'vertex shaders are not allowed to write to global temporary variables or global fields.'
                );
            }
        ...
        if (
            leftValue.getType().getCategory() === TypeCategory.HostObjectReference &&
        ...
        for (let prop of node.properties) {
            if (
                ...
                if (prop.kind === ts.SyntaxKind.PropertyAssignment) {
                    ...
                    let val = this.derefIfPointer(
                        this.extractVisitorResult(this.dispatchVisit(propAssign.initializer))
                    );
    ...
    protected handleRenderingBuiltinFunction(
        funcText: string,
        ...
        if (this.isBuiltinFunctionWithName(funcText, 'outputVertex')) {
            ...
            this.assertNode(
                node,
                this.startedVertex && !this.finishedVertex,
                'outputVertex() can only be used inside a vertex-for'
            );
        ...
        if (this.isBuiltinFunctionWithName(funcText, 'outputPosition')) {
            ...
            this.assertNode(
                node,
                this.startedVertex && !this.finishedVertex,
                'outputPosition() can only be used inside a vertex-for'
            );
            this.assertNode(node, this.currentRenderPipelineParams !== null, '[Compiler bug]');

            this.assertNode(
                node,
                posOutput.getType().getCategory() === TypeCategory.Vector,
                'position output must be a vector'
            );
            ...
            this.assertNode(
                node,
                outputVecType.getPrimitiveType() === PrimitiveType.f32,
                'position output must be a 4D f32 vector'
            );
        ...
        if (this.isBuiltinFunctionWithName(funcText, 'clearColor')) {
            ...
            this.assertNode(
                ...
                'clearColor() must have exactly 2 arguments, one for cleared texture, the other for the clear value'
            );
            ...
            this.assertNode(
                ...
                "the first argument of clearColor() must be a texture object that's visible in kernel scope"
            );
            ...
            this.assertNode(
                node,
                clearValue.getType().getCategory() === TypeCategory.Vector,
                'clear value must be a vector'
            );
            ...
            this.assertNode(
                node,
                clearValue.isCompileTimeConstant(),
                'color clear value must be a compile-time constant'
            );
        ...
        if (this.isBuiltinFunctionWithName(funcText, 'useDepth')) {
            ...
            this.assertNode(
                ...
                'useDepth() must have 1 or 2 arguments. The 1st argument should be the depth texture, the 2nd optional arguments contain depth write and clear options'
            );

            this.assertNode(
                node,
                argumentValues[0].getType().getCategory() === TypeCategory.HostObjectReference &&
                    argumentValues[0].hostSideValue instanceof DepthTexture,
                "the first argument of useDepth() must be a depth texture object that's visible in kernel scope"
            );
            let depthTexture = argumentValues[0].hostSideValue as DepthTexture;

            this.assertNode(
                node,
                this.renderPassParams!.depthAttachment === null,
                'the depth texture has already been specified'
            );
            ...
            if (node.arguments.length === 2) {
                ...
                this.assertNode(
                    node,
                    depthOptionsValue.isCompileTimeConstant(),
                    'depth options needs to be a compile time constant'
                );
                let depthOptionsType = depthOptionsValue.getType();
                this.assertNode(
                    node,
                    depthOptionsType.getCategory() === TypeCategory.Struct,
                    'depth options needs to be a struct: {storeDepth: bool, clearDepth: bool}'
                );
                ...
                if (structType.hasProperty('storeDepth')) {
                    ...
                    this.assertNode(
                        node,
                        storeDepthType.getCategory() === TypeCategory.Scalar &&
                            TypeUtils.getPrimitiveType(storeDepthType) === PrimitiveType.i32,
                        'storeDepth needs to be a boolean or integer (0 or 1)'
                    );
                ...
                if (structType.hasProperty('clearDepth')) {
                    ...
                    this.assertNode(
                        ...
                        storeDepthType.getCategory() === TypeCategory.Scalar &&
                            TypeUtils.getPrimitiveType(storeDepthType) === PrimitiveType.i32,
                        'clearDepth needs to be a boolean or integer (0 or 1)'
                    );
        ...
        if (this.isBuiltinFunctionWithName(funcText, 'outputColor')) {
            ...
            this.assertNode(
                node,
                node.arguments.length === 2,
                'outputColor() must have exactly 2 arguments, one for output texture, the other for the output value'
            );
            this.assertNode(
                node,
                argumentValues[0].getType().getCategory() === TypeCategory.HostObjectReference &&
                    isTexture(argumentValues[0].hostSideValue),
                "the first argument of outputColor() must be a texture object that's visible in kernel scope"
            );
            let targetTexture = argumentValues[0].hostSideValue as TextureBase;
            let targetLocation = this.ensureColorAttachment(targetTexture);
            ...
            this.assertNode(
                node,
                fragOutput.getType().getCategory() === TypeCategory.Vector,
                'frag output must be a vector'
            );
            let outputVecType = fragOutput.getType() as VectorType;
            this.assertNode(
                node,
                outputVecType.getNumRows() === 1 ||
                    outputVecType.getNumRows() === 2 ||
                    outputVecType.getNumRows() === 4,
                'output vector component count must be 1, 2, or 4'
            );
            this.assertNode(
                node,
                outputVecType.getPrimitiveType() === PrimitiveType.f32,
                'position output must be a f32 vector'
            );
        ...
        if (this.isBuiltinFunctionWithName(funcText, 'outputDepth')) {
            ...
            this.assertNode(
                node,
                depthOutput.getType().getCategory() === TypeCategory.Scalar,
                'depth output must be a scalar'
            );
            let outputScalarType = depthOutput.getType() as ScalarType;
            this.assertNode(
                node,
                outputScalarType.getPrimitiveType() === PrimitiveType.f32,
                'depth output must be a f32 scalar'
            );
        ...
        if (this.isBuiltinFunctionWithName(funcText, 'textureSample')) {
            ...
            this.assertNode(
                node,
                node.arguments.length === 2,
                'textureSample() must have exactly 2 arguments, one for texture, the other for the coordinates'
            );
            this.assertNode(
                node,
                argumentValues[0].getType().getCategory() === TypeCategory.HostObjectReference &&
                    isTexture(argumentValues[0].hostSideValue),
                "the first argument of textureSample() must be a texture object that's visible in kernel scope"
            );
        ...
        if (this.isBuiltinFunctionWithName(funcText, 'textureSampleLod')) {
            ...
            this.assertNode(
                ...
                'textureSampleLod() must have exactly 3 arguments, one for texture, one for the coordinates, one for the explicit LOD'
            );
            this.assertNode(
                node,
                argumentValues[0].getType().getCategory() === TypeCategory.HostObjectReference &&
                    isTexture(argumentValues[0].hostSideValue),
                "the first argument of textureSampleLod() must be a texture object that's visible in kernel scope"
            );
            let texture = argumentValues[0].hostSideValue as TextureBase;
            if (texture instanceof CanvasTexture || texture instanceof DepthTexture) {
        ...
        if (this.isBuiltinFunctionWithName(funcText, 'textureSampleCompare')) {
            ...
            this.assertNode(
                node,
                node.arguments.length === 3,
                'textureSampleComapre() must have exactly 3 arguments, one for texture, one for the coordinates, one for the depth reference'
            );
            this.assertNode(
                node,
                argumentValues[0].getType().getCategory() === TypeCategory.HostObjectReference &&
                    argumentValues[0].hostSideValue instanceof DepthTexture,
                "the first argument of textureSampleComapre() must be a depth texture object that's visible in kernel scope"
            );
            ...
            this.assertNode(
                ...
                'textureSampleCompare() cannot be used on multi-sampled depth textures'
            );
            ...
            this.assertNode(
                node,
                depthRef.getType().getCategory() === TypeCategory.Scalar &&
                    TypeUtils.getPrimitiveType(depthRef.getType()) === PrimitiveType.f32,
                'depth ref must be a scalar float'
            );

            let sampleResultStmt = this.irBuilder.create_texture_sample_compare(
                texture,
                coords.stmts,
                depthRef.stmts[0]
            );
        ...
        if (this.isBuiltinFunctionWithName(funcText, 'textureLoad')) {
            this.assertNode(
                node,
                node.arguments.length === 2,
                'textureLoad() must have exactly 2 arguments, one for texture, the other for the coordinates'
            );
            this.assertNode(
                node,
                argumentValues[0].getType().getCategory() === TypeCategory.HostObjectReference &&
        ...
        if (this.isBuiltinFunctionWithName(funcText, 'textureStore')) {
            this.assertNode(
                node,
                node.arguments.length === 3,
                'textureStore() must have exactly 3 arguments, one for texture, one for the coordinates, and one for the texel value'
            );
            this.assertNode(
                node,
                argumentValues[0].getType().getCategory() === TypeCategory.HostObjectReference &&
        ...
        if (this.isBuiltinFunctionWithName(funcText, 'dpdx') || this.isBuiltinFunctionWithName(funcText, 'dpdy')) {
            ...
            this.assertNode(
                node,
                TypeUtils.isTensorType(valType) && TypeUtils.getPrimitiveType(valType) === PrimitiveType.f32,
                'dpdx()/dpdy() must accept a f32 scalar/vector/matrix argument'
            );
    ...
    protected override visitCallExpression(node: ts.CallExpression): VisitorResult<Value> {
        ...
        let getArgumentRefs = (): Value[] => {
            ...
            for (let rawVal of rawArgumentValues) {
                if (rawVal.getType().getCategory() === TypeCategory.Pointer) {
                    ...
                    rawVal.getType().getCategory() === TypeCategory.Function ||
        ...
        if (calledFunctionValue.getType().getCategory() === TypeCategory.Function) {
            ...
            this.assertNode(
                node,
                calledFunctionValue.hostSideValue instanceof ParsedFunction,
                '[Compiler Bug] could not find parsed function'
            );
            ...
            let result = compiler.runInlining(
                parsedFunction,
                this.kernelScope,
                getArgumentRefs(),
    ...
    protected override visitElementAccessExpression(node: ts.ElementAccessExpression): VisitorResult<Value> {
        ...
        this.assertNode(
            node,
            argType.getCategory() === TypeCategory.Scalar || argType.getCategory() === TypeCategory.Vector,
            'index must be scalar or vector'
        );
        ...
        if (baseType.getCategory() === TypeCategory.HostObjectReference) {
            if (baseValue.hostSideValue instanceof Field) {
                ...
                this.assertNode(
                    node,
                    argumentValue.stmts.length === field.dimensions.length,
            ...
            if (Array.isArray(baseValue.hostSideValue)) {
                this.assertNode(
                    node,
                    argumentValue.isCompileTimeConstant(),
                    `index for accessing a host-side array must be compile-time evaluable. Bad index: ${node.argumentExpression.getText()}`
                );
                this.assertNode(
                    node,
                    argType.getPrimitivesList().length === 1,
                    'can only use 1D access on host-side arrays'
                );
        ...
        this.assertNode(
            node,
            argumentValue.isCompileTimeConstant(),
            'Indices of vectors/matrices must be a compile-time constant'
        );
        ...
        if (TypeUtils.isValueOrPointerOfCategory(baseType, TypeCategory.Vector)) {
            this.assertNode(
                node,
                argType.getPrimitivesList().length === 1,
                'index for a vector must have only 1 component'
            );
            ...
            if (argType.getCategory() === TypeCategory.Vector) {
                ...
                this.assertNode(
                    node,
                    argVecType.getNumRows() === 2,
                    'a vector index of matrix have exactly two components'
                );
    ...
    protected override visitPropertyAccessExpression(node: ts.PropertyAccessExpression): VisitorResult<Value> {
        ...
        if (objExpr.getText() === 'Math') {
            ...
            this.errorNode(
                node,
                'unrecognized Math constant: ' + node.getText() + '. Only Math.PI and Math.E are supported'
            );
        }
    ...
    protected visitVariableDeclaration(node: ts.VariableDeclaration): VisitorResult<Value> {
        ...
        if (
            initValue.getType().getCategory() !== TypeCategory.Function &&
    ...
    protected override visitIfStatement(node: ts.IfStatement): VisitorResult<Value> {
        ...
        this.assertNode(
            node,
            condValue.getType().getCategory() === TypeCategory.Scalar,
            'condition of if statement must be scalar'
        );
        ...
        if (node.expression.kind === ts.SyntaxKind.CallExpression) {
            ...
            if (
                this.isBuiltinFunctionWithName(funcText, 'static') ||
                this.isBuiltinFunctionWithName(funcText, 'Static')
            ) {
                isStaticIf = true;
            }
        ...
        if (isStaticIf) {
            this.assertNode(
                node,
                condValue.isCompileTimeConstant(),
                'if(ti.static(...)) requires a compile-time constant condition'
            );
    ...
    protected override visitBreakStatement(node: ts.BreakStatement): VisitorResult<Value> {
        this.assertNode(
            node,
            this.loopStack.length > 0 && this.loopStack[this.loopStack.length - 1] === LoopKind.While,
            'break can only be used in a while loop'
        );
        ...
        this.assertNode(
            ...
            this.loopStack.length > 0 &&
                (this.loopStack[this.loopStack.length - 1] === LoopKind.For ||
                    this.loopStack[this.loopStack.length - 1] === LoopKind.While),
            'continue must be used inside a non-static loop'
        );
        if (
            this.loopStack[this.loopStack.length - 1] === LoopKind.VertexFor ||
    ...
    protected override visitWhileStatement(node: ts.WhileStatement): VisitorResult<Value> {
        ...
        this.assertNode(
            node,
            condValue.getType().getCategory() === TypeCategory.Scalar,
            'condition of while statement must be scalar'
        );
    ...
    protected visitRangeFor(
        indexSymbols: ts.Symbol[],
        rangeExpr: ts.NodeArray<ts.Expression>,
        body: ts.Statement,
        ...
        if (shouldUnroll) {
            this.assertNode(
                null,
                rangeLengthValue.isCompileTimeConstant(),
                'for static range loops, the range must be a compile time constant'
            );
    ...
    protected visitNdrangeFor(
        ...
        for (let lengthExpr of rangeExpr) {
            ...
            this.assertNode(
                null,
                value.getType().getCategory() === TypeCategory.Scalar,
                'each arg to ndrange() must be a scalar'
            );
        ...
        if (shouldUnroll) {
            ...
            for (let len of lengthValues) {
                this.assertNode(
                    null,
                    len.isCompileTimeConstant(),
                    'for static ndrange loops, each range must be a compile time constant'
                );
    ...
    protected visitVertexFor(
        indexSymbols: ts.Symbol[],
        vertexArgs: ts.NodeArray<ts.Expression>,
        body: ts.Statement
    ): VisitorResult<Value> {
        ...
        this.currentRenderPipelineParams = new RenderPipelineParams(
            new VertexShaderParams(),
            new FragmentShaderParams()
        );
        this.ensureRenderPassParams();
        if (vertexArgs.length === 0) {
            this.errorNode(null, 'Expecting vertex buffer and optionally index buffer');
        }
        let argumentValues: Value[] = vertexArgs.map((expr: ts.Expression) =>
            this.derefIfPointer(this.extractVisitorResult(this.dispatchVisit(expr)))
        );
        if (argumentValues.length >= 1) {
            this.assertNode(
                null,
                argumentValues[0].getType().getCategory() === TypeCategory.HostObjectReference &&
                    argumentValues[0].hostSideValue instanceof Field,
                `the vertex buffer ${vertexArgs[0].getText()} must be an instance of taichi field that's visible in kernel scope`
            );
        ...
        if (argumentValues.length >= 2) {
            this.assertNode(
                null,
                argumentValues[1].getType().getCategory() === TypeCategory.HostObjectReference &&
                    argumentValues[1].hostSideValue instanceof Field,
        ...
        if (vertexArgs.length >= 3) {
            this.assertNode(
                null,
                argumentValues[2].getType().getCategory() === TypeCategory.HostObjectReference &&
                    argumentValues[2].hostSideValue instanceof Field,
                `the indirect buffer ${vertexArgs[0].getText()} must be an instance of taichi field that's visible in kernel scope`
            );
        ...
        if (vertexArgs.length >= 5) {
            this.errorNode(
                null,
                'Expecting up to 4 arguments (vertex buffer, index buffer, indirect buffer, indirect count) in inputVertices'
            );
        }
    ...
    protected visitFragmentFor(
        indexSymbols: ts.Symbol[],
        fragmentArgs: ts.NodeArray<ts.Expression>,
        body: ts.Statement
    ): VisitorResult<Value> {
        if (!this.isAtTopLevel()) {
    ...
    protected override visitForOfStatement(node: ts.ForOfStatement): VisitorResult<Value> {
        this.assertNode(
            ...
            'Expecting a `let` variable declaration list, got ',
...
export class InliningCompiler extends CompilingVisitor {
    ...
    runInlining(
        ...
        kernelScope: Scope,
        argValues: Value[],
        parentFunctionSymbolTable: SymbolTable | null = null
    ): Value | null {
    ...
    protected override registerArguments(args: ts.NodeArray<ts.ParameterDeclaration>) {
        this.assertNode(
            null,
            args.length === this.argValues.length,
    ...
    protected override visitVertexFor(
        indexSymbols: ts.Symbol[],
        vertexArgs: ts.NodeArray<ts.Expression>,
        body: ts.Statement
    ): VisitorResult<Value> {
    ...
    protected override visitFragmentFor(
        indexSymbols: ts.Symbol[],
        fragmentArgs: ts.NodeArray<ts.Expression>,
        body: ts.Statement
    ): VisitorResult<Value> {
...
export class KernelCompiler extends CompilingVisitor {
    ...
    compileKernel(
        ...
        scope: Scope,
        argTypesMap: Map<string, Type>,
        templateArgumentValues: Map<string, any> | null = null
    ): KernelParams {
        ...
        for (let i = 0; i < offloadedModules.length; ++i) {
            ...
            if (offload.type === OffloadType.Fragment) {
                previousStageBindings =
                    this.renderPipelineParams[currentRenderPipelineParamsId].vertex.bindings.slice();
            }
            let codegen = new CodegenVisitor(
                Program.getCurrentProgram().runtime!,
                offload,
                argBytes,
                returnBytes,
                previousStageBindings
    ...
    protected override registerArguments(args: ts.NodeArray<ts.ParameterDeclaration>) {
        ...
        for (let arg of this.argTypesMap.keys()) {
            if (argNames.indexOf(arg) === -1) {
                this.errorNode(
                    args[0],
                    `Invalid argument type annotaions: the annotated argument ${arg} is not in the function argument list`
                );
            }
        }
        let argStmtId = 0;
